...
function switchSlide(state) {
    ...
    carouselShow(nextSlide);
        ...
        var nextSlide = indicators.length-1;

    // Reset all slides
        for (var i = indicators.length-1; i > 0;  i--) {
            // If current slide is active & NOT equal to last slide then decrement nextSlide
            if ((indicators[i].getAttribute('data-state') == 'active') && (i !== (indicators.length+1))) {
                nextSlide = i - 1;
            }
